...
def team_required(function=None, register_period_only=False):
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            request.competition = Competition.objects.get(site_id=request.site_id)
            if request.user.is_superuser:
                if hasattr(request.user, 'teams') and request.user.team(request.competition):
                    request.team = request.user.team(request.competition)
...
def new_team_page(request):
    ...
    if request.method == 'POST':
        ...
        form = NewTeamForm(user=request.user, competition=request.competition)
...
def invite_member(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if form.member.team(request.competition):
                if form.member.team(request.competition) == request.team:
...
def accept_invite(request, slug):
    ...
    if request.user.team(request.competition):
